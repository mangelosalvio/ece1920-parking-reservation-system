...
import 'package:parking_reservation_system/profile.dart';
import 'package:parking_reservation_system/settings.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: MyHomePage(title: 'Parking Reservation'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  login({String idNo, String password}) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String ip = prefs.getString('HOST');
    String url = 'http://' + ip + ':5000/api';

    final response = await http.post(url + '/accounts/auth',
        body: {"id_no": idNo, 'password': password});
    if (response.statusCode == 200) {
      prefs.setString('id_no', json.decode(response.body)['id_no']);
